...
class ProfileRepository {
  ...
  Future<void> _ensureProfileFolder(String id) async {
    final dir = VaultDirectory(pathFor(id));
  ...
  Future<void> _migrateLegacyDataIfNeeded() async {
    ...
    final masterDir = VaultDirectory(pathFor(masterProfileId));
    var hasLegacyData = false;
    for (final folderName in ['strategy', 'budget']) {
      final legacy = VaultDirectory(p.join(vaultPath, folderName));
    ...
    for (final folderName in ['strategy', 'budget']) {
      ...
      if (await legacy.exists()) {
        final target = VaultDirectory(p.join(masterDir.path, folderName));
