...
Future<List<String>> privateRelativePathsForProfile(
  ...
  Future<void> addIfExists(String relativePath) async {
    if (await VaultFile(p.join(profileDataPath, relativePath)).exists()) {
  ...
  Future<void> addMatchingFiles(
    ...
    final dir = VaultDirectory(p.join(profileDataPath, dirRelativePath));
    if (!await dir.exists()) return;
    for (final entity in await dir.list()) {
      if (entity is VaultFile && matches(p.basename(entity.path))) {
