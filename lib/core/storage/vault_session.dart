...
class VaultSession {
  ...
  /// Id (`SavedVault.id`) du vault auquel appartient [current] — sur le
  /// build web, plusieurs coffres-forts distincts peuvent partager le même
  /// [vaultPath] virtuel (`/Opime` ou `/`, voir `vault_fs_web.dart` : la
  /// vraie racine de stockage est un handle `FileSystemDirectoryHandle`
  /// interne, pas reflétée dans ce chemin) — sans cet id, un changement de
  /// coffre-fort web vers un autre au même chemin apparent ne serait pas
  /// détecté par `main.dart`'s `_initProfiles`, et la clé de l'ancien
  /// resterait posée pour (dé)chiffrer les fichiers du nouveau.
  static String? vaultId;
}
