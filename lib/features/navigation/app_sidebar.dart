...
class AppSidebar extends StatelessWidget {
  ...
  Widget _buildItem(BuildContext context, NavItem item) {
    ...
    // Réservé à Opime Premium dans cette édition gratuite : l'item reste
    // visible et cliquable (un clic affiche un aperçu figé de la vraie
    // page, voir `LockedFeatureScreen`), mais grisé + un petit cadenas pour
    // le distinguer au premier coup d'œil des fonctionnalités débloquées.
  ...
  Widget _buildParentWithFilteredChildren(
    ...
    // Ex : le groupe "Formation" de l'Académie — réservé à Opime Premium,
    // voir `premium_lock.dart`. Grisé comme les items simples, mais reste
    // dépliable : ses enfants (déjà tous verrouillés eux-mêmes) affichent
    // chacun leur propre aperçu figé au clic.
    final locked = isPremiumLocked(item.key);
    ...
    return _withTooltip(
      ...
      NavigationCollapsible(
        leading: Icon(item.icon, color: locked ? mutedColor : null),
        label: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            shadcn.Text(itemLabel, style: locked ? TextStyle(color: mutedColor) : null),
            if (locked) ...[
              const SizedBox(width: 6),
              Icon(LucideIcons.lock, size: 12, color: mutedColor),
            ],
  ...
  Widget _buildCollapsedParentFlyout(
    ...
    return _withTooltip(
      locked
          ? '${navLocalizedLabel(l10n, item.key, fallback: item.label)} — réservé à Opime Premium'
      ...
      Builder(
        ...
        builder: (anchorContext) => NavigationItem(
          ...
          child: Icon(
            item.icon,
            color: locked
                ? Theme.of(anchorContext).colorScheme.mutedForeground
                : null,
          ),
        ),
      ),
      ...
      builder: (context, _) {
        ...
        return NavigationRail(
          ...
          children: [
            ...
            // voir `features/entities/`) n'a pas de groupe de navigation
            // propre : tout le patrimoine, personnel comme professionnel,
            // se consulte depuis le Dashboard lui-même (une carte par
            // entité, "Moi-même" comprise) — voir `dashboard_screen.dart`'s
            // section entités, qui expose son propre bouton "Gérer les
            // entités" (et "+ Ajouter une entité" tant qu'aucune n'existe
            // encore) plutôt qu'une entrée de sidebar permanente.
