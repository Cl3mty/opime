...
class CommuneBoundariesRepository {
  ...
  VaultFile _fileFor(String deptCode) =>
      VaultFile(p.join(vaultPath, 'simulations', 'commune_boundaries', '$deptCode.json'));
  ...
  Future<void> save(String deptCode, List<CommuneBoundary> boundaries) async {
    final dir = VaultDirectory(p.join(vaultPath, 'simulations', 'commune_boundaries'));
