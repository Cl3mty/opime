...
class _VaultCardState extends State<_VaultCard> {
  ...
  Future<void> _addVault() async {
    final l10n = AppLocalizations.of(context);
    // Un vrai dossier disque est la seule option web (voir la doc de
    // `OnboardingScreen._pick` — même garde-fou ici) : sans l'API File
    // System Access, impossible de garantir un stockage fiable, donc pas de
    // repli silencieux vers la zone privée du navigateur.
    if (kIsWeb && !widget.vaultFolderService.supportsWebLocalFolder) {
      setState(() => _error = l10n.core_ui_web_folder_required_detail);
      return;
    }
    // Capturé avant tout `await`, pour éviter d'utiliser `context` après un
    // trou asynchrone (voir la remarque de `mounted` plus bas).
    final dialogTitle = l10n.settings_vault_pick_dialog_title;
    ...
    try {
      final vault = kIsWeb
          ? await widget.vaultFolderService.pickAndCreateWebLocalFolderVault()
          : await widget.vaultFolderService.pickAndRememberVault(
