...
/// Une année de simulation d'une rente programmée (assurance-vie).
///
/// Chaque année, le retrait prélève une part proportionnelle du capital et des
/// gains du contrat (part du gain constante), reste soumis à l'abattement
/// annuel de l'assurance-vie (4 600 € avant 8 ans, 9 200 € ensuite) puis au
/// PFU sur le gain qui le dépasse.
class PFURenteYear {
  /// Numéro de l'année simulée (commence à 1).
  final int yearNumber;

  /// Date du retrait annuel.
  final DateTime date;

  /// Ancienneté du contrat en années à la date du retrait.
  final double seniorityYears;

  /// Abattement annuel appliqué cette année (4 600 ou 9 200 €).
  final double abattement;

  /// Rente brute versée pour l'année (12 × rente mensuelle).
  final double withdrawal;

  /// Part du gain contenue dans [withdrawal].
  final double gain;

  /// Gain imposable après déduction de l'abattement.
  final double taxableGain;

  /// Impôt sur le revenu (PFU) de l'année.
  final double pfuIr;

  /// Prélèvements sociaux (PFU) de l'année.
  final double pfuPs;

  /// Impôt total de l'année (IR + PS).
  final double pfuTotal;

  /// Rente nette perçue après impôt.
  final double netAfterPfu;
...
/// Résultat d'une simulation de rente programmée (assurance-vie).
class PFURenteResult {
  /// Détail de la rente année par année (tronqué si le capital est épuisé).
  final List<PFURenteYear> years;

  /// Montant total investi sur le contrat.
  final double capitalInvested;

  /// Valeur du contrat au début de la rente.
  final double capitalValue;

  /// Part du gain contenue dans chaque retrait (0..1), supposée constante.
  final double gainShare;

  /// Total de la rente brute versée.
  final double totalWithdrawn;

  /// Total des impôts (IR + PS) versés sur toute la durée simulée.
  final double totalPfu;

  /// Total net reçu après impôt.
  final double totalNet;

  /// Taux d'imposition effectif (totalPfu / totalWithdrawn × 100).
  final double effectiveRate;

  /// `true` si la rente s'est arrêtée avant la durée demandée, faute de
  /// capital restant suffisant pour verser une année complète.
  final bool capitalExhausted;

  const PFURenteResult({
    required this.years,
    required this.capitalInvested,
    required this.capitalValue,
    required this.gainShare,
    required this.totalWithdrawn,
    required this.totalPfu,
    required this.totalNet,
    required this.effectiveRate,
    required this.capitalExhausted,
  });
}

/// Simule une rente programmée sur une assurance-vie : un montant fixe
/// ([monthlyWithdrawal]) est retiré chaque mois à partir de [startDate], puis
/// imposé chaque année au PFU après l'abattement annuel de l'assurance-vie
/// (4 600 € si le contrat a moins de 8 ans, 9 200 € ensuite).
///
/// Modèle retenu :
/// - Chaque retrait prélève une part proportionnelle du capital et des gains
///   du contrat : la part du gain contenue dans la rente est donc supposée
///   constante, elle vaut ([capitalValue] − [capitalInvested]) / [capitalValue]
///   (bornée entre 0 et 1).
/// - Le capital restant est rémunéré chaque année au taux [annualPerformance]
///   (en %) avant le prélèvement annuel de 12 × [monthlyWithdrawal].
/// - La rente s'arrête dès que le capital restant ne permet plus de verser une
///   année entière : les années simulées sont alors tronquées et
///   [PFURenteResult.capitalExhausted] devient `true`.
PFURenteResult computePFURente({
  ...
  double annualPerformance = 0,
  double pfuIrRate = 12.8,
  double pfuPsRate = 18.6,
}) {
  // Part du gain prélevée par les retraits, supposée constante.
  final gainShare = capitalValue > 0
      ? max(0.0, min(1.0, (capitalValue - capitalInvested) / capitalValue))
      : 0.0;

  final annualWithdrawal = monthlyWithdrawal * 12;

  final years = <PFURenteYear>[];
  var remainingCapital = capitalValue;
  for (var i = 0; i < durationYears; i++) {
    final date = DateTime(startDate.year + i, startDate.month, startDate.day);

    // Rémunération du capital restant avant le prélèvement de l'année.
    remainingCapital *= (1.0 + annualPerformance / 100.0);

    // Capital insuffisant pour verser une année entière de rente.
    if (remainingCapital <= annualWithdrawal) break;

    final seniorityYears = max(0.0, date.difference(openingDate).inDays / 365.25);
    final abattement = seniorityYears >= 8 ? 9200.0 : 4600.0;

    final gain = annualWithdrawal * gainShare;
    final taxableGain = max(0.0, gain - abattement);
    final pfuIr = taxableGain * pfuIrRate / 100;
    final pfuPs = taxableGain * pfuPsRate / 100;
    final pfuTotal = pfuIr + pfuPs;

    years.add(PFURenteYear(
      yearNumber: i + 1,
      date: date,
      ...
      pfuPs: pfuPs,
      pfuTotal: pfuTotal,
      netAfterPfu: annualWithdrawal - pfuTotal,
    ));

    remainingCapital -= annualWithdrawal;
  }

  final totalWithdrawn = years.fold(0.0, (sum, y) => sum + y.withdrawal);
  final totalPfu = years.fold(0.0, (sum, y) => sum + y.pfuTotal);
  final totalNet = totalWithdrawn - totalPfu;
  final effectiveRate = totalWithdrawn > 0 ? totalPfu / totalWithdrawn * 100 : 0.0;
  final capitalExhausted = years.length < durationYears;

  return PFURenteResult(
    years: years,
    ...
    capitalExhausted: capitalExhausted,
  );
}
...
/// Mode de simulation de l'onglet PFU : un retrait unique daté, ou une rente
/// programmée (retraits périodiques, réservés à l'assurance-vie).
enum _PfuSimMode { unique, rente }
...
class _PFUTabState extends State<_PFUTab> {
  ...
  _PfuSimMode _mode = _PfuSimMode.unique;
  ...
  // --- Retrait unique : date du retrait et projection de la valeur ---
  DateTime _withdrawalDate = DateTime.now();
  double _annualGrowthRate = 0;

  // --- Rente programmée (assurance-vie) ---
  double _renteMonthly = 300;
  DateTime _renteStartDate = DateTime.now();
  int _renteDurationYears = 10;
  double _renteAnnualPerformance = 0;
  ...
  Future<void> _loadState() async {
    ...
    setState(() {
      ...
      if (modeStr == 'rente') _mode = _PfuSimMode.rente;
      final envName = data['envelope'] as String?;
      if (envName != null) {
        _envelope = AccountEnvelope.fromName(envName);
      }
      // Le mode rente n'a de sens que pour une assurance-vie.
      if (_mode == _PfuSimMode.rente && _envelope != AccountEnvelope.assuranceVie) {
        _mode = _PfuSimMode.unique;
      }
      ...
      final withdrawalDate = data['withdrawalDate'] as String?;
      if (withdrawalDate != null) {
        _withdrawalDate = DateTime.tryParse(withdrawalDate) ?? _withdrawalDate;
      }
      final growth = data['annualGrowthRate'];
      if (growth is num) _annualGrowthRate = growth.toDouble();
      final renteMonthly = data['renteMonthly'];
      if (renteMonthly is num) _renteMonthly = renteMonthly.toDouble();
      final renteStart = data['renteStartDate'] as String?;
      if (renteStart != null) {
        _renteStartDate = DateTime.tryParse(renteStart) ?? _renteStartDate;
      }
      final renteDur = data['renteDurationYears'];
      if (renteDur is num) _renteDurationYears = renteDur.toInt().clamp(1, 100);
      final rentePerf = data['renteAnnualPerformance'];
      if (rentePerf is num) _renteAnnualPerformance = rentePerf.toDouble();
    });
  }
  ...
  Future<void> _saveState() {
    return _stateRepo.write('taxation_pfu', {
      ...
      'annualGrowthRate': _annualGrowthRate,
      'renteMonthly': _renteMonthly,
      'renteStartDate': _renteStartDate.toIso8601String(),
      'renteDurationYears': _renteDurationYears,
      'renteAnnualPerformance': _renteAnnualPerformance,
    });
  }
  ...
  Future<void> _resetState() async {
    ...
    setState(() {
      ...
      _annualGrowthRate = 0;
      _renteMonthly = 300;
      _renteStartDate = DateTime.now();
      _renteDurationYears = 10;
      _renteAnnualPerformance = 0;
    });
  }
  ...
  /// `true` si la date de retrait choisie est dans le futur : la valeur du
  /// compte est alors projetée jusqu'à cette date (rendement annuel), et
  /// l'ancienneté du contrat est mesurée à la date de retrait.
  bool get _isFutureWithdrawal => _withdrawalDate.isAfter(DateTime.now());

  /// Valeur du compte au moment du retrait : c'est la valeur actuelle sauf si
  /// le retrait est projeté dans le futur, auquel cas elle est capitalisée au
  /// taux [_annualGrowthRate] entre aujourd'hui et la date du retrait.
  double get _valueAtWithdrawal {
    if (!_isFutureWithdrawal) return _currentValue;
    final years = _withdrawalDate.difference(DateTime.now()).inDays / 365.25;
    if (years <= 0) return _currentValue;
    return _currentValue * pow(1.0 + _annualGrowthRate / 100.0, years);
  }

  /// Simulation du retrait unique : la date de retrait fixe l'ancienneté du
  /// contrat (exonération PEA / abattement AV) ainsi que la valeur retenue.
  PFUResult _computeUnique() => computePFU(
    withdrawalAmount: _withdrawalAmount,
    investedAmount: _investedAmount,
    currentValue: _valueAtWithdrawal,
    ...
    referenceDate: _withdrawalDate,
  );

  /// Simulation de la rente programmée (assurance-vie).
  PFURenteResult _computeRente() => computePFURente(
    monthlyWithdrawal: _renteMonthly,
    capitalInvested: _investedAmount,
    capitalValue: _currentValue,
    openingDate: _openingDate,
    startDate: _renteStartDate,
    durationYears: _renteDurationYears,
    annualPerformance: _renteAnnualPerformance,
    pfuIrRate: _taxParams.pfuIrRate,
    pfuPsRate: _taxParams.pfuPsRate,
  );
  ...
  Widget build(BuildContext context) {
    final l10n = AppLocalizations.of(context);
    final isRente = _mode == _PfuSimMode.rente;
    final result = _computeUnique();
    final accent = Theme.of(context).colorScheme.primary;
    final hidden = widget.amountVisibility.hidden;
    ...
    return _TaxationSplitCard(
      left: SingleChildScrollView(
        child: Column(
          ...
          children: [
            // --- Type de simulation ---
            shadcn.Text(l10n.simulations_taxation_pfu_field_simulation_mode)
                .muted()
                .small(),
            const SizedBox(height: 6),
            Row(
              children: [
                Expanded(
                  child: _PillButton(
                    label: l10n.simulations_taxation_pfu_mode_unique,
                    selected: !isRente,
                    onTap: () {
                      setState(() => _mode = _PfuSimMode.unique);
                      _saveState();
                    },
                  ),
                ),
                ...
                Expanded(
                  child: _PillButton(
                    label: l10n.simulations_taxation_pfu_mode_rente,
                    selected: isRente,
                    enabled: _envelope == AccountEnvelope.assuranceVie,
                    onTap: _envelope == AccountEnvelope.assuranceVie
                        ? () {
                            setState(() => _mode = _PfuSimMode.rente);
                            _saveState();
                          }
                        : null,
                  ),
            ...
            if (_envelope != AccountEnvelope.assuranceVie) ...[
              const SizedBox(height: 6),
              shadcn.Text(l10n.simulations_taxation_pfu_rente_only_av)
                  .muted()
                  .small(),
            ],
            const SizedBox(height: 16),
            ...
            Select<AccountEnvelope>(
              ...
              onChanged: (value) {
                ...
                setState(() {
                  _envelope = value;
                  _selectedAccountId = null;
                  // Le mode rente n'existe que pour l'assurance-vie.
                  if (_envelope != AccountEnvelope.assuranceVie) {
                    _mode = _PfuSimMode.unique;
                  }
                });
            ...
            if (isRente) ...[
              ...
              _NumberField(
                label: l10n.simulations_taxation_pfu_field_rente_monthly,
                suffix: '€',
                value: _renteMonthly,
                step: 50,
                onChanged: (v) {
                  setState(() => _renteMonthly = max(0.0, v));
                  _saveState();
                },
              ),
              const SizedBox(height: 16),
              ...
              shadcn.Text(l10n.simulations_taxation_pfu_field_rente_start_date)
                  .muted()
                  .small(),
              const SizedBox(height: 6),
              _DateField(
                date: _renteStartDate,
                onChanged: (date) {
                  setState(() => _renteStartDate = date);
                  _saveState();
                },
              ),
              const SizedBox(height: 16),
              ...
              // --- Durée de la rente ---
              _NumberField(
                label: l10n.simulations_taxation_pfu_field_rente_duration,
                suffix: 'ans',
                value: _renteDurationYears.toDouble(),
                step: 1,
                onChanged: (v) {
                  setState(() {
                    _renteDurationYears = v.round().clamp(1, 100);
                  });
                  _saveState();
                },
              ),
              ...
              _NumberField(
                label: l10n.simulations_taxation_pfu_field_rente_performance,
                suffix: '%',
                value: _renteAnnualPerformance,
                step: 0.5,
                decimals: 1,
                onChanged: (v) {
                  setState(() => _renteAnnualPerformance = v);
                  _saveState();
                },
              ),
              const SizedBox(height: 16),
              ...
              // --- Date du retrait ---
              shadcn.Text(l10n.simulations_taxation_pfu_field_withdrawal_date)
                  .muted()
                  .small(),
              const SizedBox(height: 6),
              _DateField(
              ...
              // --- Projection de la valeur jusqu'à la date du retrait ---
              if (_isFutureWithdrawal) ...[
                _NumberField(
                  label: l10n.simulations_taxation_pfu_field_annual_return,
                  suffix: '%',
                  value: _annualGrowthRate,
                  step: 0.5,
                  decimals: 1,
                  onChanged: (v) {
                    setState(() => _annualGrowthRate = v);
                    _saveState();
                  },
                ),
                const SizedBox(height: 6),
                shadcn.Text(l10n.simulations_taxation_pfu_projection_hint)
                    .muted()
                    .small(),
                const SizedBox(height: 16),
              ],
      ...
      right: isRente
          ? _buildRenteResultColumn(context)
          : Column(
        children: [
          // --- Résumé ---
          shadcn.Text.rich(
          ...
          // --- Projection : retrait futur, valeur capitalisée ---
          if (_isFutureWithdrawal) ...[
            _InfoNote(text: l10n.simulations_taxation_pfu_projection_hint),
            const SizedBox(height: 12),
            _StatRow(
              items: [
                (
                  l10n.simulations_taxation_pfu_stat_projected_value,
                  [displayEuros(_valueAtWithdrawal, hidden)],
                ),
              ],
            ),
            const SizedBox(height: 16),
  ...
  /// Colonne de résultats du mode « retrait unique » — voir [build].
  Widget _buildRenteResultColumn(BuildContext context) {
    final l10n = AppLocalizations.of(context);
    final result = _computeRente();
    final hidden = widget.amountVisibility.hidden;
    final accent = Theme.of(context).colorScheme.primary;
      ...
      children: [
        ...
        shadcn.Text.rich(
          TextSpan(
            ...
            children: [
              TextSpan(
                text: l10n.simulations_taxation_pfu_summary_withdrawal_prefix(
                  l10n.simulations_taxation_pfu_mode_rente,
                ),
              ),
              TextSpan(
                text: displayEuros(result.totalWithdrawn, hidden),
                style: TextStyle(
                  color: accent,
                  fontWeight: FontWeight.bold,
                ),
              ...
              TextSpan(text: l10n.simulations_taxation_pfu_summary_pfu_tax_prefix),
              TextSpan(
                text: displayEuros(result.totalPfu, hidden),
                style: const TextStyle(
              ...
              TextSpan(text: l10n.simulations_taxation_pfu_summary_net_received_prefix),
              TextSpan(
                text: displayEuros(result.totalNet, hidden),
                style: const TextStyle(
                  color: Color(0xFF66BB6A),
                  fontWeight: FontWeight.bold,
                ),
        ...
        // --- Stats globales de la rente ---
        _StatRow(
          items: [
            (
              l10n.simulations_taxation_pfu_stat_total_withdrawn,
              [displayEuros(result.totalWithdrawn, hidden)],
            ),
            (
              l10n.simulations_taxation_pfu_stat_total_tax,
              [displayEuros(result.totalPfu, hidden)],
            ),
            (
              l10n.simulations_taxation_pfu_stat_total_net,
              [displayEuros(result.totalNet, hidden)],
            ),
          ],
        ),
        const SizedBox(height: 12),
        _StatRow(
          items: [
            (
              l10n.simulations_taxation_pfu_stat_effective_rate,
              ['${result.effectiveRate.toStringAsFixed(2)} %'],
            ),
            (
              l10n.simulations_taxation_pfu_stat_gain_share,
              ['${(result.gainShare * 100).toStringAsFixed(1)} %'],
            ),
            (
              l10n.simulations_taxation_pfu_stat_simulated_years,
              ['${result.years.length}'],
            ),
          ],
        ),
        ...
        if (result.years.isNotEmpty) ...[
          ...
          SingleChildScrollView(
            ...
            child: _RenteTable(years: result.years, hidden: hidden),
          ),
        ],
        ...
        if (result.capitalExhausted) ...[
          ...
          if (result.years.isEmpty)
            _InfoNote(
              warn: true,
              text: l10n.simulations_taxation_pfu_rente_capital_insufficient,
            )
          else
            _InfoNote(
              warn: true,
              text: l10n.simulations_taxation_pfu_rente_capital_exhausted(
                result.years.length,
              ),
            ),
        ],

        // --- Abattement annuel & hypothèses ---
        const SizedBox(height: 16),
        _InfoNote(text: l10n.simulations_taxation_pfu_rente_abattement_legend),
        const SizedBox(height: 8),
        _InfoNote(text: l10n.simulations_taxation_pfu_rente_assumptions),
...
/// Note explicative « info » ou d'avertissement sous forme de bandeau discret.
class _InfoNote extends StatelessWidget {
  final String text;
  final bool warn;

  const _InfoNote({required this.text, this.warn = false});

  @override
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final iconColor =
        warn ? const Color(0xFFE9A23B) : theme.colorScheme.mutedForeground;
    return Container(
      ...
      child: Row(
        ...
        children: [
          Icon(
            warn ? LucideIcons.triangleAlert : LucideIcons.info,
            size: 16,
            color: iconColor,
          ),
          const SizedBox(width: 10),
...
/// Tableau année par année d'une rente programmée. Les largeurs de colonnes
/// sont fixes pour garder les montants alignés ; le tableau est déroulant
/// horizontalement si la largeur disponible est insuffisante.
class _RenteTable extends StatelessWidget {
  final List<PFURenteYear> years;
  final bool hidden;

  const _RenteTable({required this.years, required this.hidden});
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        Row(
          children: [
            cell(l10n.simulations_taxation_pfu_table_year,
                width: 56, header: true),
            cell(l10n.simulations_taxation_pfu_table_contract_age,
                width: 92, header: true),
            cell(l10n.simulations_taxation_pfu_table_withdrawal,
                width: 108, header: true),
            cell(l10n.simulations_taxation_pfu_table_tax,
                width: 108, header: true),
            cell(l10n.simulations_taxation_pfu_table_net,
                width: 108, header: true),
          ],
        ),
        const Divider(height: 1),
        for (final y in years)
          Row(
            children: [
              cell('${y.yearNumber}', width: 56),
              cell('${y.seniorityYears.toStringAsFixed(1)} ans', width: 92),
              cell(displayEuros(y.withdrawal, hidden), width: 108),
              cell(
                displayEuros(y.pfuTotal, hidden),
                width: 108,
                color: y.pfuTotal > 0 ? const Color(0xFFE07A6B) : null,
              ),
              cell(
                displayEuros(y.netAfterPfu, hidden),
                width: 108,
                color: const Color(0xFF66BB6A),
              ),
            ],
