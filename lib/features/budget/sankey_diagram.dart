...
class SankeyDiagram extends StatelessWidget {
  ...
  /// Échelle euros -> pixels de hauteur des nœuds/liens. `0.10` (défaut,
  /// comportement historique inchangé pour les deux écrans Budget réels)
  /// convient à un vrai écran ; une valeur plus petite permet de faire
  /// tenir le MÊME diagramme (mêmes nœuds/liens/montants, juste plus
  /// compact) dans un espace réduit — voir la vitrine web
  /// (`features/landing/landing_screen.dart`), qui a besoin d'une
  /// reproduction fidèle du vrai diagramme sans l'espace d'un écran
  /// complet.
  final double pixelsPerEuro;

  /// Hauteur minimale du canevas, avant marge (voir [_layout]). `300`
  /// (défaut, comportement historique inchangé) évite qu'un tout petit
  /// budget produise un diagramme ridiculement plat sur un vrai écran ;
  /// une valeur plus petite est nécessaire pour la même raison que
  /// [pixelsPerEuro] dans un espace réduit.
  final double minCanvasHeight;
  ...
  const SankeyDiagram({
    ...
    this.emptyMessage,
    this.pixelsPerEuro = 0.10,
    this.minCanvasHeight = 300,
  });
  ...
  double _layout(List<SankeyNode> nodes, List<SankeyLink> links, double width) {
    ...
    double maxColumnHeight = minCanvasHeight;
