...
/// utile dans la vue "Par avoir" (les lignes y sont mélangées entre
...
/// La plus ancienne de deux dates éventuellement absentes — `null` compte
/// comme "pas de date", jamais comme "plus ancienne que toute date réelle"
/// (sinon une seule liste vide court-circuiterait l'autre). Partagé par
/// `dashboard_screen.dart` et `entities/entities_patrimoine_adapter.dart`
/// pour combiner la date de transaction la plus ancienne (comptes) et la
/// date de départ de prêt la plus ancienne (passifs) en une seule borne
/// "Tout" pour [assetClassHistoriesFor]/`totalPassifHistoryFor`.
DateTime? earliestOfDates(DateTime? a, DateTime? b) {
  if (a == null) return b;
  if (b == null) return a;
  return a.isBefore(b) ? a : b;
}

/// Historique "Patrimoine net/brut" par classe d'actif réelle, sur une
/// grille commune bornée par [period]/[earliest] — factorisé pour être
/// partagé par le patrimoine personnel (`dashboard_screen.dart`'s
/// `_actifsHistoryFor`) et celui de chaque entité
/// (`entities/entities_overview_section.dart`/`entity_detail_screen.dart`),
/// qui n'en diffèrent que par la liste de [accounts] passée en entrée —
/// voir la doc de [investmentsForEffectiveClass]/
/// [leveragedValueForEffectiveClass] pour le détail du calcul par point.
///
/// [excludeFlagged] (`false` par défaut) suit la même règle que
/// [investmentsForEffectiveClass] : seul le patrimoine personnel global l'a
/// à `true` (le comportement historique de cette fonction, avant qu'elle ne
/// soit partagée avec les entités). Une entité comptabilise TOUJOURS tout
/// (comme une page de catégorie) — sans ce paramètre, sa courbe "Patrimoine"
/// omettait silencieusement les comptes marqués "hors patrimoine global",
/// alors que [entityNetValue] (la carte "Valeur nette") les compte : deux
/// nombres différents pour la même entité sur le même écran (voir la doc de
/// tête d'`entity_detail_screen.dart`).
Map<String, List<NetWorthPoint>> assetClassHistoriesFor(
  List<InvestmentAccount> accounts,
  Map<String, List<PricePoint>> priceHistories,
  DashboardPeriod period,
  DateTime? earliest, {
  ...
  final start = period.startFor(today: today, earliest: earliest ?? today);
