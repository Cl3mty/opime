...
class AutresPhotoRepository {
  ...
  VaultDirectory get _dir =>
      VaultDirectory(p.join(vaultPath, 'investissements', 'autres', 'photos'));

  VaultFile get _indexFile => VaultFile(
  ...
  Future<String?> photoPathFor(String investmentId) async {
    ...
    final file = VaultFile(p.join(_dir.path, entry.file));
  ...
  Future<String?> importPhoto(
    ...
    final file = VaultFile(p.join(_dir.path, fileName));
