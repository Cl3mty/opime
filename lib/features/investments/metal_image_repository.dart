...
class MetalImageRepository {
  ...
  VaultDirectory get _imagesDir =>
      VaultDirectory(p.join(vaultPath, 'investissements', 'metaux', 'images'));

  VaultFile get _indexFile => VaultFile(
    p.join(vaultPath, 'investissements', 'metaux', 'images.json'),
  );

  VaultFile fileFor(String fileName) => VaultFile(p.join(_imagesDir.path, fileName));
