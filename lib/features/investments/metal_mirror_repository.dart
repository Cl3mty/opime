...
class MetalMirrorRepository {
  ...
  VaultDirectory get _root =>
      VaultDirectory(p.join(vaultPath, 'investissements', 'metaux_precieux'));
  ...
  Future<void> _reconcile(
    ...
    if (await _root.exists()) {
      for (final metalDir in (await _root.list()).whereType<VaultDirectory>()) {
        ...
        for (final dateDir
            in (await metalDir.list()).whereType<VaultDirectory>()) {
        ...
        if ((await metalDir.list()).isEmpty) await metalDir.delete();
      }
    }

    if (desired.isEmpty) {
      if (await _root.exists() && (await _root.list()).isEmpty) {
        await _root.delete();
      }
      return;
    }
    ...
    for (final metal in desired.keys) {
      final metalDir = VaultDirectory(p.join(_root.path, metal));
      for (final day in desired[metal]!.keys) {
        final dayDir = VaultDirectory(p.join(metalDir.path, day));
  ...
  Future<void> _syncDay(
    ...
    for (final entry in entries) {
      ...
      for (final doc in docs) {
        ...
        final target = VaultFile(p.join(dayDir.path, targetName));
    ...
    final transactionFile = VaultFile(p.join(dayDir.path, 'transaction.json'));
    ...
    for (final file in (await dayDir.list()).whereType<VaultFile>()) {
