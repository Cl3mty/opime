...
enum _Step {
  // Uniquement si le coffre-fort a déjà au moins une entité (voir `_load`)
  // : à qui appartient ce qui va être créé — une entité existante, ou une
  // toute nouvelle. Toujours la toute première étape dans ce cas (avant
  // même `kind`), sauf si un raccourci `initial*` fait déjà démarrer le
  // flux plus loin (voir la doc de [showCompletePatrimoineDialog]) — ces
  // raccourcis restent des flux personnels, cohérents avec la page d'où
  // ils sont ouverts. Un coffre-fort sans aucune entité encore créée saute
  // directement cette étape (comportement identique à avant l'existence du
  // module Entités) : la première entité se crée depuis l'écran "Entités"
  // (toujours accessible, quel que soit le coffre-fort), pas ici.
  owner,
  kind,
  assetClass,
  account,
...
/// Peut aussi créer une dette (prêt immobilier, crédit autre) : la
/// première étape laisse choisir "Avoir" ou "Dette", chacun menant à son
/// propre sous-flux (avoir : classe → compte → investissement →
/// transaction ; dette : type → formulaire).
///
...
class _CompletePatrimoineDialogState extends State<_CompletePatrimoineDialog> {
  ...
  /// Entités déjà créées dans ce coffre-fort (reste vide s'il n'en a
  /// aucune), résolu dans [_load] — proposées à [_Step.owner] (qui n'existe
  /// que si cette liste n'est pas vide, voir [_load]) et utilisées pour
  /// peupler le sélecteur "Détenue par" de l'éditeur ouvert depuis là.
  List<BusinessEntity> _entities = [];

  /// Entité choisie à [_Step.owner] — propriétaire du compte/passif que ce
  /// flux va créer. `null` si le coffre-fort n'a encore aucune entité
  /// (aucune étape [_Step.owner] n'existe alors), ou quand le flux a été
  /// ouvert via un raccourci `initial*` (reste un flux personnel, voir la
  /// doc de l'enum `_Step`).
  ...
  Future<void> _load() async {
    ...
    final entities = await _entityRepo.listAll();
    if (!mounted) return;
    setState(() {
      _accounts = accounts;
      _entities = entities;
      _loading = false;
      // Le flux "vierge" (aucun raccourci `initial*` — `_step` encore à sa
      // valeur par défaut `_Step.kind` à ce stade, `initState` s'exécutant
      // avant `_load`) démarre par le choix de l'entité propriétaire,
      // plutôt que directement sur "Que voulez-vous compléter ?", dès que
      // ce coffre-fort a au moins une entité — voir la doc de l'enum
      // `_Step`.
      if (entities.isNotEmpty && _step == _Step.kind) {
        _step = _Step.owner;
      }
    });
    ...
    switch (_step) {
      ...
      case _Step.kind:
        return _KindStep(
          ...
          // cette étape a réellement été précédée de [_Step.owner] (une
          // entité existe déjà, sans raccourci `initial*`) — sans entité,
          // ou via un raccourci, [_Step.kind] reste la toute première étape
          // comme avant.
          onBack: _entities.isNotEmpty
...
/// Toute première étape dès que ce coffre-fort a déjà au moins une entité
/// (voir `_load`) : choisir le propriétaire de ce qui va être créé, avant
/// même de choisir "Un avoir"/"Une dette" — "Moi-même" reste toujours la
/// première option, pour qu'un coffre-fort qui mélange personnel et
/// entités professionnelles n'oblige jamais à rattacher un nouveau
/// compte/passif à une entité (voir la doc de tête de
/// [showCompletePatrimoineDialog]).
class _OwnerStep extends StatelessWidget {
  final List<BusinessEntity> entities;
  final VoidCallback onSelectMyself;
  final ValueChanged<String> onSelectEntity;
  final VoidCallback onCreateEntity;

  const _OwnerStep({
    required this.entities,
    required this.onSelectMyself,
    ...
    return Column(
      ...
      children: [
        ...
        _OptionTile(
          ...
          label: l10n.investments_wizard_owner_myself_label,
          sublabel: l10n.investments_wizard_owner_myself_sublabel,
          onTap: onSelectMyself,
        ),
...
class _KindStep extends StatelessWidget {
  ...
  /// été précédée de ce choix (au moins une entité existe, flux non
  /// raccourci), voir l'appelant dans `_buildStep`.
  final VoidCallback? onBack;
