...
class CategoryDetailScreen extends StatefulWidget {
  ...
  /// Par avoir" de l'Allocation et n'affiche que [category.accounts] tel
  /// quel. Quand renseigné, [category.accounts] est utilisé comme vue "Par
  /// avoir" (une ligne par investissement, ex : Google/Meta/Nvidia) et ce
  ...
  /// Titre du tableau des comptes de la catégorie — "Placements" par
  /// défaut, "Dettes" pour les catégories de crédits/emprunts (voir
  /// `RealPassifDetailScreen`). `null` retombe sur le libellé localisé
  /// "Placements"/"Investments" — un défaut littéral figé en français
  /// n'aurait pas suivi le changement de langue de l'app.
...
class _CategoryDetailScreenState extends State<CategoryDetailScreen> {
  ...
  /// (vue "par avoir") est déjà un investissement quand elle n'a pas de
...
class _CategoryAllocationCard extends StatefulWidget {
  ...
  /// "Par avoir" à la place de [category.accounts] quand renseigné (Actions
  ...
  /// deux parts distinctes du graphique, même en vue "Par avoir").
...
class _CategoryAllocationCardState extends State<_CategoryAllocationCard> {
  ...
  Widget build(BuildContext context) {
    ...
    // Vue "Par avoir" de l'épargne : une même devise (EUR, USD...) détenue
...
/// Tableau "Placements" de la page de détail d'une classe. Quand [byAccount]
...
class _AccountAvatar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (imagePath == null || imagePath.isEmpty) {
      final avatar = _initials(context);
    ...
    final avatarWithImage = VaultImageAvatar(
      imagePath: imagePath,
      size: 28,
