...
// Même correctif que le toggle "Par compte"/"Par avoir" de l'Allocation
...
/// Carte "Placements" ou "Dettes" du Dashboard : une ligne par catégorie
...
/// pour l'Allocation de la page de détail d'une classe d'avoir. Sans
/// [categoriesByInvestment] (Dettes, un prêt n'a pas de "positions" à
...
class CategoryBreakdownCard extends StatefulWidget {
  ...
  /// Remplace la navigation par défaut (`NavigationScope.call(category.id)`,
  /// qui route vers la page de la classe d'actif RÉELLE, personnelle) — sans
  /// substitut, une carte affichant les catégories d'UNE entité routerait
  /// vers cette même page personnelle, qui ne montre jamais les comptes/
  /// passifs de l'entité (filtrés par `entityId`) : la page semblerait vide
  /// à tort (voir `EntitiesOverviewSection`, seul appelant à fournir ce
  /// callback aujourd'hui).
  final ValueChanged<String>? onCategoryTap;

  /// Rend chaque ligne de compte/investissement dépliée sous une catégorie
  /// cliquable (par défaut, une ligne de compte n'est qu'informative — voir
  /// [_AccountRow]) : `null` la laisse non cliquable, comme sur le
  /// Dashboard, où seule la catégorie route quelque part. Utilisé par
  /// `entity_detail_screen.dart`, qui n'a pas de page de catégorie dédiée où
  /// faire un second saut — cliquer un compte doit donc ouvrir directement
  /// son détail depuis cet accordéon.
            ...
            Expanded(
              child: MouseRegion(
                ...
                child: GestureDetector(
                  onTap: () => (onCategoryTap ?? NavigationScope.maybeOf(context))
                      ?.call(category.id),
                  child: Padding(
                    ...
                    children: [
                      for (final account in category.accounts)
                        _AccountRow(
                          ...
                          showPnl: showPnl,
                          period: period,
                          onTap: onAccountTap == null || account.id == null
                              ? null
...
class _AccountRow extends StatelessWidget {
  ...
  /// `null` : ligne purement informative (comportement historique, voir
  /// [CategoryBreakdownCard.onAccountTap]).
  ...
  Widget build(BuildContext context) {
    ...
    return MouseRegion(
      cursor: SystemMouseCursors.click,
      child: GestureDetector(onTap: tap, child: row),
    );
  }
}
