...
import '../investments/real_patrimoine_adapter.dart' show evenDateGrid;
...
/// Comme [totalBalanceOnGrid], mais construit lui-même sa grille à partir
/// de [period]/[earliest] — factorisé pour être partagé par le patrimoine
/// personnel (`dashboard_screen.dart`'s `_totalPassifHistoryFor`) et celui
/// de chaque entité (`entities/entities_overview_section.dart`/
/// `entity_detail_screen.dart`), même principe que
/// `real_patrimoine_adapter.dart`'s `assetClassHistoriesFor` côté actifs.
List<NetWorthPoint> totalPassifHistoryFor(
  List<Liability> liabilities,
  DashboardPeriod period,
  DateTime? earliest,
) {
  ...
  return totalBalanceOnGrid(liabilities, evenDateGrid(start, today));
}
