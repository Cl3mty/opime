...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      // Le sondage de `pumpScreen` s'arrête dès l'apparition du bouton
      // "Ajouter" (toujours présent), alors que la *lecture* réelle du mois
      // sur disque et son rendu sont un travail asynchrone séparé qui peut
      // finir une frame plus tard — on attend donc explicitement le champ
      // persistant (même motif de sondage `runAsync` que `pumpScreen`).
      await tester.runAsync(() async {
        for (var i = 0; i < 40 && findBudgetField().evaluate().isEmpty; i++) {
          await Future<void>.delayed(const Duration(milliseconds: 50));
          await tester.pump();
        }
      });
