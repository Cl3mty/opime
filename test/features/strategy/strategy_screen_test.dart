...
void main() {
  ...
  /// Attend que [condition] devienne vraie, en pompant dans la zone réelle
  /// (`runAsync`) — voir [tapAndSettle] et `pumpScreen` : les mutations ci-
  /// dessous enchaînent de vraies E/S et finissent par un rechargement
  /// d'état (`_loadFolders`) dont la *dernière* affectation de notifier peut
  /// tomber juste après la dernière frame sondée ; on ne peut pas se
  /// contenter d'une durée fixe qui trancherait en plein milieu. Plafond de
  /// 2 s : au-delà, l'assertion qui suit signale un vrai problème, pas un
  /// simple retard.
  ...
  Future<void> moveNoteToFolder(WidgetTester tester, String folderName) async {
    ...
    // La note doit avoir quitté "Sans dossier" à l'écran : ce n'est pas
    // qu'une question d'affichage, c'est ce qui garantit que
    // `_folderState` a bien été rechargé (et donc que le duplicata d'un
    // test ultérieur sera rangé dans le bon dossier, voir
    // `StrategyScreen._duplicateNote`).
    await waitUntil(
      tester,
      () => find.text('Sans dossier').evaluate().isEmpty,
    );
  }
  ...
  testWidgets('renommer un dossier met à jour son nom affiché', (
    ...
    await waitUntil(
      tester,
      () => find.text('Impôts 2026').evaluate().isNotEmpty,
    );
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await waitUntil(
        tester,
        () => find.text('Impôts').evaluate().isEmpty,
      );
      expect(find.text('Impôts'), findsNothing);
