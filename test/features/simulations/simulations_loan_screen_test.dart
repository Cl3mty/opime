...
void main() {
  group('simulateLoan — prêt amortissable classique', () {
    ...
    test('le capital est intégralement remboursé sur la durée du prêt', () {
      ...
      final capitalRembourse = result.years.fold<double>(
        0,
        (s, y) => s + y.capital * 12,
      );
    ...
    test(
      'le coût total avec frais additionne bien les frais de dossier et de garantie',
      () {
        final result = simulateLoan(
        ...
        expect(
          result.coutTotalAvecFrais,
          closeTo(result.coutTotalCredit + 2000, 0.01),
        );
  ...
  group('simulateLoan — prêt in fine', () {
    test(
      'les intérêts sont constants et le capital est remboursé en une fois',
      () {
        final result = simulateLoan(
        ...
        expect(
          result.years.first.interest,
          closeTo(result.years.last.interest, 0.01),
        );
        final expectedMonthlyInterest = 150000 * (4.0 / 100 / 12);
        expect(
          result.years.first.interest,
          closeTo(expectedMonthlyInterest, 0.01),
        );
  ...
  group('simulateLoan — différé de remboursement', () {
    test(
      'franchise partielle : seuls les intérêts sont payés, le capital ne bouge pas',
      () {
        final result = simulateLoan(
          montantEmprunte: 100000,
          dureeAnnees: 20,
    ...
    test(
      'franchise totale : rien n\'est décaissé hors assurance (intérêts capitalisés)',
      () {
        final result = simulateLoan(
          montantEmprunte: 100000,
          dureeAnnees: 20,
    ...
    test(
      'franchise totale : le capital restant dû augmente avec les intérêts capitalisés',
      () {
        ...
        expect(
          withTotalDefer.coutTotalCredit,
          greaterThan(withoutDefer.coutTotalCredit),
        );
      },
    );
  });
