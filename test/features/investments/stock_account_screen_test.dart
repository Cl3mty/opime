...
void main() {
  ...
  testWidgets(
    'la table des positions n\'affiche pas d\'identifiant pour un '
    ...
    '"Autres" avec une vraie référence saisie',
    (tester) async {
      await tester.runAsync(() async {
        tempDir = await Directory.systemTemp.createTemp(
          'opime_stock_account_test',
  ...
  testWidgets(
    'la popup d\'un objet "Autres" affiche un avatar cliquable pour '
    'importer une photo (initiales par défaut)',
    (tester) async {
      await tester.runAsync(() async {
        tempDir = await Directory.systemTemp.createTemp(
          'opime_stock_account_test',
  ...
  testWidgets(
    'un fonds détenu en PER, sans ISIN saisi, se voit générer un '
  ...
  testWidgets(
    'un fonds PEG sans ISIN (Actions & Fonds) affiche aussi le cours '
    'manuel une fois renseigné, comme "Autres"',
    (tester) async {
      await tester.runAsync(() async {
        tempDir = await Directory.systemTemp.createTemp(
          'opime_stock_account_test',
  ...
  testWidgets(
    'créer une toute nouvelle position depuis l\'onglet Transactions '
  ...
  testWidgets(
    'ajouter une transaction sur un fonds PEG affiche sa date de '
    ...
    (tester) async {
      ...
      final defaultUnlockDate = DateTime(
        today.year + 5,
        today.month,
        today.day,
      );
