...
void main() {
  ...
  group(
    ...
    () {
      ...
      localizationsDelegates: [
        ...
        await tester.runAsync(() async {
          for (var i = 0; i < 40; i++) {
            if (find.text('Une dette').evaluate().isNotEmpty) return;
            await Future<void>.delayed(const Duration(milliseconds: 50));
            await tester.pump();
          }
        });
        await tester.tap(find.text('Une dette'));
        await tester.pump();
      }
      ...
      testWidgets(
        ...
        (tester) async {
          ...
          await tester.pumpAndSettle();
          await tester.ensureVisible(find.text('Créer la dette'));
          await tester.pumpAndSettle();
  ...
  group(
    'Étape préalable "À qui appartient ceci ?" (coffre-fort avec des '
    'entités — aucune distinction personnel/professionnel de coffre-fort '
    'n\'existe, la présence d\'au moins une entité suffit à déclencher '
    'cette étape)',
    () {
    ...
    testWidgets(
      'aucune entité créée dans ce coffre-fort : le flux démarre '
      'directement sur "Que voulez-vous compléter ?", pas d\'étape '
      'préalable',
      (tester) async {
        ...
        expect(find.text('Un avoir'), findsOneWidget);
        expect(find.text('Une dette'), findsOneWidget);
    ...
    testWidgets(
      'sélectionner "Moi-même" passe directement à "Que voulez-vous '
      'compléter ?" — le compte créé ensuite n\'est rattaché à aucune '
      'entité',
      (tester) async {
        await tester.runAsync(
          () => EntityRepository(tempDir.path).saveEntity(
        ...
        await tester.tap(find.text('Moi-même'));
        await tester.pump();
    ...
    testWidgets(
      'une entité existante : elle apparaît comme tuile, la sélectionner '
      'passe directement à "Que voulez-vous compléter ?"',
      (tester) async {
        await tester.runAsync(
          () => EntityRepository(tempDir.path).saveEntity(
            const BusinessEntity(
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        await tester.tap(find.text('Holding Dupont'));
        await tester.pump();
        await tester.tap(find.text('Un avoir'));
        await tester.pump();
    ...
    testWidgets(
      ...
      '"Que voulez-vous compléter ?" entièrement, même avec une entité '
      'déjà créée',
      (tester) async {
        await tester.runAsync(
          () => EntityRepository(tempDir.path).saveEntity(
      ...
      localizationsDelegates: [
        ...
        await tester.runAsync(() async {
          for (var i = 0; i < 40; i++) {
            if (find.text('Créer la dette').evaluate().isNotEmpty) return;
            await Future<void>.delayed(const Duration(milliseconds: 50));
            await tester.pump();
          }
        });
