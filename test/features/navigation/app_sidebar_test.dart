...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      // Le groupe Entités (toujours affiché, voir `entitesGroup` dans
      // `nav_models.dart`) pousse Budget plus bas dans la sidebar,
      // au point de le sortir de la fenêtre de test par défaut —
      // `ensureVisible` avant de taper, comme pour tout item désormais
      // hors champ sans scroller.
  ...
  testWidgets(
    'le groupe "Entités" n\'a pas de nav dédiée — tout le patrimoine, '
    'personnel comme professionnel, se consulte depuis le Dashboard '
    '(une section par entité), pas via la sidebar',
    (tester) async {
      await pumpSidebar(tester, collapsed: false, onSelect: (_) {});
