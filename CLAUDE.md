...
Opime is a local-first personal finance app (net worth tracking, budget, financial simulations) built with Flutter. No backend, no database: every account's data is stored as plain JSON and Markdown files inside an `Opime` folder the user picks on first launch (can live in iCloud Drive, Dropbox, etc). A hidden `.opime` subfolder inside it is reserved for future software-level config, distinct from user data. Desktop (macOS) is the primary target; Windows/Linux are supported; mobile builds are not yet configured despite `android/`/`ios/` scaffolding existing.

**This repository (`opime`, public — formerly `opime-free`) is the free, open-source (AGPL-3.0) edition, forked from the private `opime-premium` codebase (formerly just `opime`).** It is the same app and the same code, minus a small feature-gating layer (see "Premium feature gating" below) that locks a handful of screens behind a fictitious "Opime Premium" upgrade. Everything under the Dashboard — unlimited accounts, all asset/liability classes, Budget, Stratégie, Simulation, Académie Fondamentaux/Enveloppes — stays fully free and unlocked. **Do not remove the gating layer or unlock a locked feature without the user's explicit consent** — that's the entire point of this fork existing separately from `opime-premium`.

### Premium feature gating

**The real source for locked features does not live in this repository at all** — not just visually hidden behind a client-side check, which anyone could strip out of an AGPL-licensed app by deleting a wrapper widget and recompiling. Analyses, Projets, Assistant IA, the "Gérer les entités" management screen, and Académie > Formation's lesson content were deleted from this fork; each locked screen is instead a small, purpose-built mockup with illustrative/fake data and no real calculation logic, living at the same file path and class name the real feature used to have (so `main.dart`'s imports/usages barely changed) — see e.g. `lib/features/analyses/analyses_screen.dart`, `lib/features/projects/projects_screen.dart`, `lib/features/assistant/assistant_screen.dart`, `lib/features/entities/entities_screen.dart`, `lib/features/academy/formation_track_screen.dart` + `formation_data.dart` (now just track id/title/icon metadata, no lesson steps). **When asked to add or restore a locked feature's real functionality, do not just remove the lock UI — that would require actually re-implementing the feature from scratch, which is exactly the point.**

- `lib/core/premium/premium_lock.dart` defines `premiumLockedKeys` (the `NavItem.key`/`pages` map keys reserved for Premium: `analyses`, `projets`, `entites`, `assistant`, `formation` and its five track ids) and `premiumUpgradeUrl`.
- `lib/core/premium/locked_feature_screen.dart`'s `LockedFeatureScreen` wraps a locked page's (mock) builder: renders it inert (`IgnorePointer`) and blurred/dimmed behind a card explaining the limitation with a button to `premiumUpgradeUrl`. Wired into the relevant entries of `main.dart`'s `pages` map.
- `lib/core/premium/premium_mock_widgets.dart`'s `PremiumMockCard`/`PremiumMockScreen` are the shared building blocks for those mockups (icon + title + fake caption, in a scrollable column) — reuse them for consistency rather than hand-rolling a new mockup layout.
- `app_sidebar.dart` and `mobile_nav_hub.dart` grey out (muted color + small lock icon) any nav item whose key is in `premiumLockedKeys`, while keeping it clickable (a click still navigates to the key, which resolves to a `LockedFeatureScreen` via `main.dart`'s `pages` map — nothing needs to be duplicated at the call site).
- **Entités is a partial exception**: the data model (`entities_models.dart`), repository, `entities_patrimoine_adapter.dart`, `entity_ownership_summary.dart`, and `entity_detail_screen.dart` (read-only view) all stay — they're load-bearing for the free Dashboard's per-entity display and for `investments_models.dart`/`liabilities_models.dart`'s `entityId` field (no personal/professional distinction exists at the data-model level, see `app_sidebar.dart`'s doc comment). Only the actual creation/management UI (`entities_screen.dart`'s real implementation, `entity_structure_diagram.dart`) was removed. The inline "create a new entity" step in `complete_patrimoine_dialog.dart` (`_OwnerStep`'s "Nouvelle société" tile — the user-facing name for entities is "Sociétés", not "Entités"; only code identifiers keep the latter — reached only when a vault already has at least one entity) shows a locked toast instead of opening a real editor — see `_createEntityAndSelect`.
- The Assistant's "enabled in Settings" toggle, unread-response badge, and toast-on-background-response no longer exist — Assistant is unconditionally locked in this edition, so that whole runtime-state machinery (once split across `main.dart`, `app_shell.dart`, `app_sidebar.dart`, `settings_screen.dart`) was removed along with `core/assistant/` (the real LLM clients — Anthropic/OpenAI/Google/Ollama — and context builder) rather than kept around unused.
- Adding a new locked feature: delete its real implementation, add a same-path mockup (reusing `PremiumMockCard`/`PremiumMockScreen` where it fits), add its key(s) to `premiumLockedKeys`, and wrap its `pages` map entry in `main.dart` with `LockedFeatureScreen`. The sidebar/mobile-hub greying is automatic since it looks up `isPremiumLocked` by key. Also check `test/` for suites covering the real implementation you're deleting — they need to go (or be rewritten against the new locked/mock behavior) too, see `test/global_search_index_test.dart` and `test/features/investments/complete_patrimoine_dialog_test.dart` for examples of the latter.
- The GitHub owner/repo used by the update checker (`main.dart`'s `_githubOwner`/`_githubRepo`) and by the landing page's release/discussions links (`landing_screen.dart`) point at `Cl3mty/opime` (this repo), not `Cl3mty/opime-premium` — keep it that way so this build checks its own releases, not Premium's.
