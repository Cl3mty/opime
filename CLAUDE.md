...
- `lib/core/premium/locked_feature_screen.dart`'s `LockedFeatureScreen` wraps a locked page's real builder: it still constructs the real screen (so the preview is faithful to real data), but renders it inert (`IgnorePointer`) and blurred/dimmed behind a card explaining the limitation with a button to `premiumUpgradeUrl`. Wired into the relevant entries of `main.dart`'s `pages` map.
...
- Adding a new locked feature: add its key(s) to `premiumLockedKeys`, wrap its `pages` map entry in `main.dart` with `LockedFeatureScreen`. The sidebar/mobile-hub greying is automatic since it looks up `isPremiumLocked` by key.
