...
**This repository (`opime-free`) is the free, open-source (AGPL-3.0) edition, forked from the private `opime`/`opime-premium` codebase.** It is the same app and the same code, minus a small feature-gating layer (see "Premium feature gating" below) that locks a handful of screens behind a fictitious "Opime Premium" upgrade. Everything under the Dashboard — unlimited accounts, all asset/liability classes, Budget, Stratégie, Simulation, Académie Fondamentaux/Enveloppes — stays fully free and unlocked. **Do not remove the gating layer or unlock a locked feature without the user's explicit consent** — that's the entire point of this fork existing separately from `opime`.
...
- The GitHub owner/repo used by the update checker (`main.dart`'s `_githubOwner`/`_githubRepo`) and by the landing page's release/discussions links (`landing_screen.dart`) point at `Cl3mty/opime-free`, not `Cl3mty/opime` — keep it that way so this build checks its own releases, not Premium's.
